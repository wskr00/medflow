...
| `GET/POST /api/unidades`, `/consultorios`, `/especialidades`, `/medicos` | Administração autenticada; criação 201 com id/version; consultas paginadas na clínica configurada; `GET /api/{recurso}/{id}` retorna o detalhe administrativo no mesmo escopo |
...
| `GET/POST /api/regras-agenda`; `PUT /api/regras-agenda/{id}` | `medicoId`, `consultorioId`, `especialidadeId`, `diaSemana` (1 segunda a 7 domingo), `horaInicio`, `horaFim`, `duracaoMinutos`, `vigenteDe`, `vigenteAte?`, `ativo`; detalhe por id; lista filtra explicitamente por `medicoId`, `especialidadeId` e `consultorioId` |
| `GET/POST /api/bloqueios-agenda`; `PUT /api/bloqueios-agenda/{id}` | `medicoId`, `inicio`, `fim`, `ativo`; `inicio`/`fim` do payload são `date-time` local sem offset, resolvidos no fuso da clínica; resposta usa ISO 8601 com offset da clínica; detalhe por id e lista filtra explicitamente por `medicoId`; bloqueio médico absoluto; alteração revalida reservas afetadas |
...
Projeção operacional da recepção (agenda, fila, detalhe e respostas de check-in, reagendamento e cancelamento): `id`, `version`, `inicio`, `fim`, `status`, `checkInEm` (null antes da chegada), `medico`, `especialidade`, `unidade`, `consultorio`, `paciente` (cada um `{id,nome}`), `allowedActions:{canCheckIn,canReschedule,canCancel}` e `pendenteDeDiaAnterior`. As ações são calculadas pelo backend a partir do estado atual e do relógio da clínica; a UI não as infere pelo seu próprio relógio. `canCheckIn` é verdadeiro somente para Recepção, em `AGENDADA`, quando a data local da consulta é hoje (chegada antecipada no mesmo dia é permitida). `canReschedule` e `canCancel` são verdadeiros somente em `AGENDADA` futura. `pendenteDeDiaAnterior` é verdadeiro somente em item `EM_ESPERA` iniciado antes de hoje no fuso da clínica. Nenhum texto clínico integra essa projeção. A projeção do Paciente não inclui `paciente`, `canCheckIn` ou `pendenteDeDiaAnterior`. Disponibilidade inclui também esses objetos de exibição, além dos IDs de seleção, evitando requisições individuais por slot.
